...
class Agent0:
    ...
    def fsm(self):
        while not rospy.is_shutdown():
            ...
            if not self.mycore.ID == 1:
                if (time.time() - self.start) > 750:
                    if self.simulation.abrupt == 0:
                        if (time.time() - self.start) > (750 + (self.mycore.ID - 1) * 30):
                            self.mycore.willingness = [1.0, 0.0]
                            self.mycore.delta = 0.0
                            self.mycore.gamma = 1.0
                    elif self.simulation.abrupt == 1:
                        self.mycore.willingness = [1.0, 0.0]
                        self.mycore.delta = 0.0
                        self.mycore.gamma = 1.0
                    else:
                        msg = '[fsm %d] abrupt has weird value\n'
                        rospy.loginfo(msg)
